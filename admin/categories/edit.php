...
try {
    ...
    if (!$category) {
        echo '<div class="container mt-3"><div class="alert alert-danger">Danh mục không tồn tại!</div></div>';
            ...
            if ($stmt->fetchColumn() > 0) {
                ...
                if ($has_category_content) {
                    if ($has_category_sort_order) {
                        $stmt = $pdo->prepare("UPDATE categories SET name = ?, slug = ?, description = ?, content = ?, parent_id = ?, status = ?, sort_order = ?, meta_title = ?, meta_description = ?, meta_keywords = ?, focus_keyword = ?, og_image = ? WHERE id = ?");
                        $stmt->execute([$name, $slug, $description, $content, $parent_id, $status, $sort_order, $meta_title, $meta_description, $meta_keywords, $focus_keyword, $og_image, $id]);
                    } else {
                        $stmt = $pdo->prepare("UPDATE categories SET name = ?, slug = ?, description = ?, content = ?, parent_id = ?, status = ?, meta_title = ?, meta_description = ?, meta_keywords = ?, focus_keyword = ?, og_image = ? WHERE id = ?");
                        $stmt->execute([$name, $slug, $description, $content, $parent_id, $status, $meta_title, $meta_description, $meta_keywords, $focus_keyword, $og_image, $id]);
                    }
                } else {
                    if ($has_category_sort_order) {
                        $stmt = $pdo->prepare("UPDATE categories SET name = ?, slug = ?, description = ?, parent_id = ?, status = ?, sort_order = ?, meta_title = ?, meta_description = ?, meta_keywords = ?, focus_keyword = ?, og_image = ? WHERE id = ?");
                        $stmt->execute([$name, $slug, $description, $parent_id, $status, $sort_order, $meta_title, $meta_description, $meta_keywords, $focus_keyword, $og_image, $id]);
                    } else {
                        $stmt = $pdo->prepare("UPDATE categories SET name = ?, slug = ?, description = ?, parent_id = ?, status = ?, meta_title = ?, meta_description = ?, meta_keywords = ?, focus_keyword = ?, og_image = ? WHERE id = ?");
                        $stmt->execute([$name, $slug, $description, $parent_id, $status, $meta_title, $meta_description, $meta_keywords, $focus_keyword, $og_image, $id]);
                    }
                }

                if (function_exists('log_activity')) {
                    log_activity('update', 'category', $id, "Cập nhật danh mục: $name");
                }
...
$seo_data = [
    ...
    'preview_title' => $category['name'],
    'preview_url' => BASE_URL . 'danh-muc/' . $category['slug']
];
$current_category_url = categoryUrl($category['slug'], true);
?>
                    ...
                    <form method="POST" action="">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="name"
                                ...
                                onkeyup="if(!document.getElementById('slug').value) document.getElementById('slug').value = createSlug(this.value)">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-text mt-1">
                                Link danh mục:
                                <a id="category-url-preview" href="<?php echo e($current_category_url); ?>" target="_blank"
                                    class="text-break text-decoration-none">
                                    <?php echo e($current_category_url); ?>
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="parent_id" name="parent_id">
                                ...
                                <?php echo render_category_options($pdo, $category['parent_id'], $id, false); ?>
                            </select>
                        </div>
                        ...
                        <?php if ($has_category_content): ?>
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="content" name="content" rows="10"><?php echo e($category['content'] ?? ''); ?></textarea>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const urlPreview = document.getElementById('category-url-preview');
        const categoryBaseUrl = <?php echo json_encode(rtrim(categoryUrl('', true), '/')); ?>;

        const updateCategoryUrlPreview = () => {
            if (!slugInput || !urlPreview) return;
            const slug = (slugInput.value || '').trim();
            const fullUrl = slug ? `${categoryBaseUrl}/${slug}` : `${categoryBaseUrl}/...`;
            urlPreview.textContent = fullUrl;
            if (slug) {
                urlPreview.href = fullUrl;
                urlPreview.classList.remove('disabled');
            } else {
                urlPreview.href = 'javascript:void(0)';
                urlPreview.classList.add('disabled');
            }
        };

        if (slugInput && urlPreview) {
            slugInput.addEventListener('input', updateCategoryUrlPreview);
            updateCategoryUrlPreview();
        }
    });
</script>
